...
LBTY_tenuErrorStatus LMD_u8Display(u8* const pu8Display){
	...
	for(u8 i = LMD_MAX_ROW_NUM ; i-- ; ){
		GPIO_u8SetPortValue  (LMD_ROW_PORT, pu8Display[i]);
		GPIO_u8TogglePinValue(LMD_COL_PORT, i);
		vidMyDelay_ms(LMD_DELAY);
		//GPIO_u8TogglePinValue(LMD_COL_PORT, i);
	...
	for(u8 i = LMD_MAX_COL_NUM ; i-- ; ){
		...
		for(u8 j = LMD_MAX_COL_NUM ; j-- ; ){
			u8ColChar |= GET_BIT(pu8Display[j], i)<<(j);
		}

		GPIO_u8SetPortValue  (LMD_COL_PORT, ~u8ColChar);
...
LBTY_tenuErrorStatus LMD_u8Char(u8 u8Char){
	LBTY_tenuErrorStatus  u8RetErrorState = LBTY_OK;
	for(u8 i = LMD_CHAR_RATE ; i-- ; ){
		if((u8RetErrorState = LMD_u8Display((u8*)IMAGES[u8Char]))){
			break;
		}
	}
	return u8RetErrorState;
...
LBTY_tenuErrorStatus LMD_u8String(u8* pu8Str){
	LBTY_tenuErrorStatus  u8RetErrorState = LBTY_OK;
	while(*pu8Str){
		if((u8RetErrorState = LMD_u8Char(*pu8Str))){
			break;
		}
		if((u8RetErrorState = LMD_u8Char(' '))){
			break;
		}
		pu8Str++;
	}
...
/* Return      :	LBTY_tenuErrorStatus									  */
/* ************************************************************************** */
void LMD_vidSetString(u8* pu8Str){
	u8 i = 0;
	for( ; *pu8Str ; i++){
...
LBTY_tenuErrorStatus LMD_u8Update(void){
	...
	static u8 pu8Char[LMD_MAT_LEN] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

	if(u8Refresh++ > LMD_REFRESH_RATE){
		u8Refresh =  LBTY_u8ZERO;
		u8 u8TempChar = pu8Array_GLB[u8index];
		for(u8 i = 0 ; i<LMD_MAT_LEN ; i++){
			pu8Char[i] = pu8Char[i] >> 1 | (GET_BIT(IMAGES[u8TempChar][i], u8Shift)) << (LMD_MAT_LEN - 1);
		}
		if(u8Shift<LMD_MAT_LEN){
			u8Shift++;
		}else{
			u8Shift = LBTY_u8ZERO;
